...
class Coach:
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				# print(self.aging_loss.extract_ages(x))
				# print('=' * 100)
				...
				# 	loss = torch.tensor(loss.item(), device=self.device, requires_grad=True)
				####
				loss.backward()
				...
				if self.global_step % self.opts.image_interval == 0 or \
					...
					self.parse_and_log_images(id_logs, x, y, y_hat, y_recovered, title='images')
	...
	def calc_loss(self, x, y, y_hat, latent, target_ages, input_ages, no_aging, data_type="real"):
		...
		# 수정한 코드
		# return torch.tensor(loss, device=self.device), loss_dict, id_logs
